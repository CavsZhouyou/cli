 ...
 * @Descriptions: npm scripts 执行（包含生命周期 hook）
 * @TodoList: 无
 * @Date: 2021-03-23 20:47:00 
 * @Last Modified by: xiaotian@tangping
 * @Last Modified time: 2021-03-23 21:11:37
 */
...
const runScriptPkg = async options => {
  ...
  if (options.cmd)
    ...
    // NodeGypPackage 时添加默认的 install 命令 node-gyp rebuild
    cmd = defaultGypInstallScript
  else if (event === 'start' && await isServerPackage(path))
    // 添加默认的 start 命名为 node server.js
